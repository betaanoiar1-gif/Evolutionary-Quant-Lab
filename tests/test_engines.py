...
def market(n=500):
    rng=np.random.default_rng(9); close=100*np.cumprod(1+rng.normal(.0002,.008,n)); ts=pd.date_range("2021-01-01",periods=n,freq="h",tz="UTC")
    return pd.DataFrame({"timestamp":ts,"open":close,"high":close*1.005,"low":close*.995,"close":close,"volume":100})
...
def test_funding_changes_equity():
    df=market(); d=StrategyDNA(10,50)
...
def test_meta_strategy_selects_from_candidates():
    df=market(); strategies=[StrategyDNA(10,50),StrategyDNA(20,100)]
    chosen,regime=MetaStrategy(strategies).choose(df)
